...
# 三种不同的步长计算方法
def compute_step1(delta, k=0.6, max_step=0.02):#距离固定比例,靠近后是delta主导，k增益不变
        """
        delta: np.ndarray (3,)
        return: step (3,)
        """
...
def compute_step2(delta, #距离自适应比例 需要大步长，靠近后是delta主导，增益k会随距离变化
                k_max=0.8,
                max_step=0.06,
                slow_radius=0.02):
    """
    slow_radius: 开始减速的距离 (m)
    """
    d = np.linalg.norm(delta[:2])  # 平面距离

    if d < 1e-6:
        return np.zeros_like(delta)

    k = min(k_max, d / slow_radius)
...
def compute_step3(delta,
                max_step=0.05,
                slow_radius=0.02):
    
    def smooth_gain(d, d0):
        t = np.clip(d / d0, 0.0, 1.0)
        return 3 * t**2 - 2 * t**3
    ...
    gain = smooth_gain(d, slow_radius)
    step = gain * delta
    step = np.clip(step, -max_step, max_step)
    return step


def compute_step_stable(delta,
                        max_step=0.01,   # 最大步长 1cm
                        slow_radius=0.04,# 开始减速的距离 4cm
                        k_max=0.8,       # 最大增益
                        deadband=0.0005   # 0.5mm 死区
                        ):
    """
    稳定动态步长计算函数
    输入：
...
class Aubo_Robot(Auboi5Robot):
    def __init__(self, robot_host_ip="192.168.1.40", robot_port=8899,is_connect=True ,workspace_limits=None, #
        ...
        if (self.is_use_camera):
            ...
            # self.cam_intrinsics = np.array([602.61245237, 0, 321.43089928, 0, 602.94485939, 250.42390681, 0, 0, 1]).reshape(3, 3)
...
if __name__ == "__main__":
    ...
    robot = Aubo_Robot(is_use_camera=False)
